...
做 PPT 分三步，各由一个 skill 负责：设计与制作之间以一份**《逐页设计稿》**交接，制作完成后配讲稿：
...
| **`ppt-script`**（讲稿） | 做 PPT 最后一步。定稿 pptx 后，读设计稿 + 成品页 → 逐页**口语化讲稿**（别念稿 / 凝练 / 页间过渡）→ 经你审 → 写进幻灯片备注。 |
...
skills/
  ...
  ppt-script/    讲稿：SKILL.md + references(讲稿铁律 / 讲稿模板 / 个人讲稿风格占位) + scripts/write_notes.py
...
- ✅ geely 公司风（封面 / 结尾 / 中间页）
- ✅ 三 skill 流程：`ppt-design` 设计 → 审批 → `ppt-skills` 制作 → `ppt-script` 讲稿（写进备注）→ 回灌
